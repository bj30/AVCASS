...
class UNet2d(nn.Module):
    def __init__(
        ...
        if visual_feat_dim is None:
            ...
            self.vid_feature_aligner_tkn_conv = nn.Conv2d(1, 1,(4, 1),(4, 1), bias=False)
            self.vid_feature_aligner_tkn_conv.weight.data.zero_()
            self.vid_feature_aligner_tkn_linear = nn.Linear(64, visual_feat_dim)
            # zero out the tkn_linear
    ...
    def forward(self, x, t, mixture_latents, vid=None):
        ...
        if isinstance(vid, Tuple):
            vid_feats_1 = self.vid_feature_aligner(vid[0])
            vid_feats_2 = self.vid_feature_aligner_tkn_conv(vid[1].unsqueeze(1)).squeeze(1)
            vid_feats_2 = self.vid_feature_aligner_tkn_linear(vid_feats_2)
            vid = torch.cat([vid_feats_1, vid_feats_2], dim=1)
        else:
