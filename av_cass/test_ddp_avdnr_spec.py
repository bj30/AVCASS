...
def main(mode, args):
    ...
    # Load model:
    model = SiT_models[args.model](in_channels=8, out_channels=6).to(device)
    ...
    dataset_audio_directory = args.audio_files_dir
    if args.folder_name:
        folder_name = args.folder_name
        exclude_list = []
        if '/AVDnR' in args.audio_files_dir:
            # Corrected AVDnR roots already contain file_list_test.txt plus a test/ directory.
            # In that case the dataset loader should receive the root directly instead of
            # appending another "/test" and later falling back to a bogus "/eval" path.
            if os.path.exists(os.path.join(args.audio_files_dir, "file_list_test.txt")):
                audio_directory = os.path.join(args.audio_files_dir, "test")
                dataset_audio_directory = args.audio_files_dir
            else:
                ...
                dataset_audio_directory = audio_directory
            if os.path.isdir(f"{args.sample_dir}/{folder_name}"):
                for case_name in os.listdir(f"{args.sample_dir}/{folder_name}"):
                    for file_name in os.listdir(f"{args.sample_dir}/{folder_name}/{case_name}"):
                        # check if all sources are saved
                        exists = 0
                        for stem in ["speech", "sfx", "music"]:
                            if os.path.exists(os.path.join(args.sample_dir, folder_name, case_name, file_name, f"{stem}.wav")):
                                exists += 1
                        if exists == 3:
    ...
    dataset = MultiSourceDataset(
        ...
        audio_files_dir=dataset_audio_directory,
    ...
    if rank == 0:
        ...
        for parts in os.listdir(sample_folder_dir):
            for case in os.listdir(os.path.join(sample_folder_dir, parts)):
                if os.path.isdir(os.path.join(sample_folder_dir, parts, case)):
                    for file in os.listdir(os.path.join(sample_folder_dir, parts, case)):
                        if file.endswith(".csv"):
                            df = pd.read_csv(os.path.join(sample_folder_dir, parts, case, file))
