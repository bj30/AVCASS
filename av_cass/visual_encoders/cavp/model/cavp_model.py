...
class CAVP_Inference(nn.Module):
    ...
    def encode_video(self, video, normalize: bool = False, train=False, pool=True):
        ...
        # Pooling: (max-pool, for extra contrastive)
        if pool:
    ...
    def forward(self, video, spec, pool=True, output_dict=True):
        video_features = self.encode_video(video, normalize=True, pool=pool)
        spec_features = self.encode_spec(spec, normalize=True, pool=pool)
        if output_dict:
            return {
...
class CAVP_Inference_VideoOnly(nn.Module):
