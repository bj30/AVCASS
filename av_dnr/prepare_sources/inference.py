...
def mono_check(audio): # from general_utils.py
    '''
...
class SMDetector:
    ...
    def predict_audio_fma(self, audio_path, sr=16000):
        try:
            if sr==44100:
                stereo_audio, sr = librosa.load(audio_path, sr=None, mono=False) # sr=44100
            else:
                stereo_audio, sr = librosa.load(audio_path, sr=sr, mono=False) # sr=44100
        except:
            print(f"Skipping {audio_path} due to librosa error")
            return None, None
        if stereo_audio.ndim==1:
            print(f"{audio_path} is mono!")
            stereo_audio = np.expand_dims(stereo_audio, axis=0)
        results = []
        for mono_audio in stereo_audio:
            y = np.expand_dims(mono_audio, axis=0)
            audio = torch.from_numpy(y).float()
            results.append(self.predict_audio(audio=audio, sr=sr))
        return results, stereo_audio

    def predict_audio(self, audio_path=None, audio=None, sr=16000):
        assert (audio_path is not None) or (audio is not None) # at least one should be given
        ...
        if sr != 16000:
            resample = torchaudio.transforms.Resample(sr, 16000).to(self.device)
            audio = resample(audio)
        # print(audio.shape, audio_path)
        audio_pcen_data = self.pcen_transform(audio)
        # print(audio_pcen_data.shape)
...
def result_count(result):
    n_speech, n_music, n_total = 0, 0, 0
    for r in result:
        n_total += 1
        ...
        if r['music_prob'] > music_threshold:
            n_music += 1
    return n_speech, n_music, n_total        
...
if __name__ == '__main__':
    ...
    parser.add_argument('--format', type=str, default='csv_prob', choices=['csv', 'csv_prob'])
